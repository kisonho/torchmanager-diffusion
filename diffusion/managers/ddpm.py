...
class DDPMManager(DiffusionManager[Module]):
    ...
    def forward_diffusion(self, data: torch.Tensor, **kwargs: Any) -> tuple[DiffusionData, torch.Tensor]:
        ...
        sqrt_alphas_cumprod_t = _get_index_from_list(self.beta_space.sqrt_alphas_cumprod, t, x_start.shape)
        sqrt_one_minus_alphas_cumprod_t = _get_index_from_list(self.beta_space.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
    ...
    def sampling_step(self, data: DiffusionData, i: int, /) -> torch.Tensor:
        ...
        sqrt_recip_alphas_t = _get_index_from_list(self.beta_space.sqrt_recip_alphas, data.t, data.x.shape)
        ...
        if i > 0:
            ...
            posterior_variance_t = posterior_variance_t.to(y.device)
